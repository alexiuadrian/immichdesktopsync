<script lang="ts">
  ...
  import { getThumbUrl, getCachedUrl } from '../lib/thumbCache';
  ...
  $: if (asset) imgPromise = getThumbUrl(asset.id);
  ...
  function next() { if (index < assets.length - 1) index++; }
  ...
  async function handleDownload() {
    ...
    downloadMsg = '';
    try {
      await DownloadAsset(asset.id, filename || `${asset.id}.jpg`);
      downloadMsg = 'Saved';
...
>
  ...
  <div class="absolute bottom-0 left-0 right-0 flex items-center justify-between bg-gradient-to-t from-black/70 to-transparent px-6 py-4">
    ...
    <span class="ml-4 flex-shrink-0 text-xs text-white/40">{index + 1} / {assets.length}</span>
  </div>
</div>
